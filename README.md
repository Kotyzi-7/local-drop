...
- **Zero Python dependencies** — runs with just Python's standard library
...
- The QR code feature uses [Qrious v4.0.2](https://github.com/nicedoc/qrious) (GPL v3), embedded inline. Works fully offline with no internet connection required.
- The server binds to `0.0.0.0` by default, which listens on all network interfaces. Make sure you trust the local network.
- Password-protected sessions expire after 24 hours.
- Files uploaded to the server are stored in a `shared_files/` directory next to the script.
